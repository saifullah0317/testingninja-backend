"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.QuestionPoolController = void 0;
const common_1 = require("@nestjs/common");
const guards_1 = require("../auth/guards");
const auth_guard_1 = require("../auth/auth.guard");
const auth_service_1 = require("../auth/auth.service");
const questionpool_service_1 = require("./questionpool.service");
const questionpool_dto_1 = require("./questionpool.dto");
let QuestionPoolController = class QuestionPoolController {
    constructor(questionPoolService, authService) {
        this.questionPoolService = questionPoolService;
        this.authService = authService;
    }
    async getall(req) {
        let userid = await this.authService.getUseridByToken(req.cookies);
        console.log("user id while getting attempterslists: ", userid);
        return await this.questionPoolService.getbyuserid(userid);
    }
    async addlist(req, body) {
        let tempObj = JSON.parse(JSON.stringify(body));
        tempObj.userid = await this.authService.getUseridByToken(req.cookies);
        return await this.questionPoolService.addlist(tempObj);
    }
};
exports.QuestionPoolController = QuestionPoolController;
__decorate([
    (0, common_1.UseGuards)(guards_1.JwtGuard),
    (0, common_1.UseGuards)(auth_guard_1.ExtractUser),
    (0, common_1.Get)(),
    __param(0, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], QuestionPoolController.prototype, "getall", null);
__decorate([
    (0, common_1.UseGuards)(guards_1.JwtGuard),
    (0, common_1.UseGuards)(auth_guard_1.ExtractUser),
    (0, common_1.Post)(),
    __param(0, (0, common_1.Req)()),
    __param(1, (0, common_1.Body)(new common_1.ValidationPipe())),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, questionpool_dto_1.QuestionPoolDto]),
    __metadata("design:returntype", Promise)
], QuestionPoolController.prototype, "addlist", null);
exports.QuestionPoolController = QuestionPoolController = __decorate([
    (0, common_1.Controller)('questionpool'),
    __metadata("design:paramtypes", [questionpool_service_1.QuestionPoolService, auth_service_1.AuthService])
], QuestionPoolController);
//# sourceMappingURL=questionpool.controller.js.map